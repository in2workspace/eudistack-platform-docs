---
title: Examples
description: Code examples to integrate with the EUDIStack API
---

# Examples

This page contains code examples in different languages to integrate with the EUDIStack API.
...
def request_presentation(required_claims, callback_url):
    """Requests a credential presentation."""
    response = requests.post(
...
def check_presentation_status(request_id):
    """Checks the status of a presentation request."""
...
def presentation_callback():
    ...
    if data['status'] == 'success':
        ...
        print(f"Verified user: {claims['given_name']} {claims['family_name']}")

        # Here you can create a session, update DB, etc.
        return jsonify({"status": "ok"})

    elif data['status'] == 'error':
        print(f"Verification error: {data['error']}")
...
app.post('/callback', (req, res) => {
  ...
  if (status === 'success') {
    console.log(`Verified user: ${verified_claims.given_name} ${verified_claims.family_name}`);
    // Process verified credentials
...
class EUDIStackClient:
    """Complete client for the EUDIStack API."""
    ...
    def _get_token(self) -> str:
        """Gets or renews the access token."""
...
[:material-sitemap: View system architecture](../arquitectura/index.md){ .md-button }
